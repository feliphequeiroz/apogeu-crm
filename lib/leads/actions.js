...
export async function createLead(leadData) {
  try {
    ...
    const { data: stages, error: stagesError } = await getPipelineStages(leadData.user_id)
    ...
    const newLead = {
      ...
      status: leadData.status || stages[0]?.key || slugify(stages[0]?.name),
...
/**
 * Reassign leads from one stage to another
 */
export async function reassignLeadsInStage(oldStageName, newStageName, userId) {
  try {
    if (!oldStageName || !newStageName || !userId) {
      return { data: null, error: 'Old stage name, new stage name, and user ID are required' }
    }

    const { error } = await supabaseAdmin
      .from('leads')
      .update({ status: newStageName, updated_at: new Date().toISOString() })
      .eq('status', oldStageName)
      .eq('user_id', userId)
    ...
    console.error('Error in reassignLeadsInStage:', error)
    return { data: null, error: error.message }
  }
}