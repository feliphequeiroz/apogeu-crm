...
import { updatePipelineStage } from '@/lib/pipeline/actions'
import { pipelineColors } from '@/lib/pipeline/constants'

export default function EditStageModal({ isOpen, onClose, stage, onStageUpdated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data, error: updateError } = await updatePipelineStage(stage.id, {
        name: formData.name,
        icon: formData.icon,
        color: formData.color,
        position: formData.position,
        is_visible: stage.is_visible, // Keep current visibility
      })
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto min-h-0 px-6 py-4 space-y-4">
          ...
          <div>
            <label htmlFor="position" className="block text-sm font-semibold text-gray-900 dark:text-white mb-1">Posição</label>
            <input
              type="number"
              id="position"
              name="position"
